...
- **文件源**: 支持从 `{appData}/skills/` 目录加载 `.md` 文件
- **导入导出**: 导入时自动解析 YAML frontmatter，导出一键生成标准 SKILL.md
...
![](https://i.ibb.co/gZwKqrwR/image.png)
...
![](https://i.ibb.co/tMj0dbM2/image.png)
...
2. 可以通过三种方式添加技能：
   ...
   - **文件源**: 将 `.md` 文件放入 `{appData}/skills/` 目录，自动加载到文件源列表
...
支持 YAML frontmatter 格式的 `.md` 文件导入，也支持无 frontmatter 的纯文本导入（自动使用默认元数据）。
...
## 协议与开源声明 (License & Open Source Notice)
...
>  **注意**：为了规避版权风险，本项目**不包含**任何内置的 Live2D 模型文件（`.moc3`）。用户需要自行准备合法合规的模型，并通过应用内的“模型管理”面板自行导入。用户需自行承担其所使用模型的版权责任。
...
>  **注意**：用户如果在本地自行训练并替换特定的音色权重文件（`.pth` / `.ckpt`），请务必确保用于训练的音频数据集未侵犯他人的声音版权。
...
Made with  by [FGXYX](https://github.com/FGXYX)
